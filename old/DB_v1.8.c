...
int main(int argc, char *argv[]){	
	...
	system("/root/project/position/iBeaconScanV1.8/ibeacon_scan4.sh &");	
...
void rssiDataA(int rk,int *count,double *rssi,int *start){
	...
	if(abs(RSSItemp[rk])>0 && RSSItemp[rk]>=-85){
		rssi[cc]=RSSItemp[rk];		
		cc++;
		*count=cc;
		//printf("count:%d\n",*count);
			...
			*/	
			...
			if(!breakNear){
				...
				if(near==1){	
					...
					nearWhibeacon=0;
					//是否在區間2
					int check2Area=RSSI[0]+RSSI[1];	
					//int check2AreaCER=abs(RSSI[0]-RSSI[1]);	
					if( (check2Area<=-120 && check2Area>=-165) &&
					(RSSI[2]<=range2 &&  RSSI[3]<=range2) &&
						(RSSI[0]<range1 && RSSI[1]<range1)){
						//&&check2AreaCER<centerRange){
						
						pos=2;
					}
					
					//是否在區間4
					int check4Area=RSSI[1]+RSSI[3];	
					//int check4AreaCER=abs(RSSI[1]-RSSI[3]);						
					if( (check2Area<=-120 && check2Area>=-165) && 
					(RSSI[0]<=range2 &&  RSSI[2]<=range2)&& 
					(RSSI[1]<range1 && RSSI[3]<range1)){
					//check4AreaCER<centerRange){
						
						pos=4;
					}
					
					//是否在區間6
					int check6Area=RSSI[0]+RSSI[2];	
					//int check6AreaCER=abs(RSSI[0]-RSSI[2]);	
					if( (check2Area<=-120 && check2Area>=-165) &&
					(RSSI[1]<=range2 &&  RSSI[3]<=range2)&&
					(RSSI[0]<range1 && RSSI[2]<range1)){
					//check6AreaCER<centerRange){
						
						pos=6;
					}
					
					//是否在區間8
					int check8Area=RSSI[2]+RSSI[3];	
					//int check8AreaCER=abs(RSSI[2]-RSSI[3]);
					if( (check2Area<=-120 && check2Area>=-165) && 
					(RSSI[0]<=range3 &&  RSSI[1]<=range3) &&  
					(RSSI[2]<range1 && RSSI[3]<range1)){
					//check8AreaCER<centerRange){						
						pos=8;
					}
					if( !(pos>=2 && pos<=8)){
						pos=5;		
