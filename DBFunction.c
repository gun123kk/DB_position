...
void dealOLEDData(oled *info){
	...
	*/
	...
	lineEnd=sprintf(buffer," RI%d:%d,RI%d:%d",2,info->rssi[1],1,info->rssi[0]);
	OLED_display(buffer,lineEnd,1);	
	lineEnd=sprintf(buffer," RI%d:%d,RI%d:%d",4,info->rssi[3],3,info->rssi[2]);
	OLED_display(buffer,lineEnd,2);	
	lineEnd=sprintf(buffer," area:%d         ",info->area);
	OLED_display(buffer,lineEnd,3);	
